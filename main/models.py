...
class Airline(UUIDModel):
    ...
    max_total_dimensions_carry_on = models.FloatField(_('三辺合計上限(機内持ち込み)'))
    max_total_dimensions_domestic = models.FloatField(_('三辺合計上限(国内線)'))
    max_total_dimensions_international = models.FloatField(_('三辺合計上限(国際線)'))
    max_weight_carry_on = models.FloatField(_('最大重量(機内持ち込み)'))
    max_weight_domestic = models.FloatField(_('最大重量(国内線)'))
    max_weight_international = models.FloatField(_('最大重量(国際線)'))
    order = models.SmallIntegerField(_('表示順'))
    is_supported = models.BooleanField(_('対応中'), default=True)
    created_at = models.DateTimeField(_('作成日時'), auto_now_add=True)
    updated_at = models.DateTimeField(_('更新日時'), auto_now=True)
